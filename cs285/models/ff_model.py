...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        #self.update_statistics(obs_mean, obs_std, acs_mean, acs_std, delta_mean, delta_std) #it changes stats to tensor and results in error in normalize()
        ...
        next_obs_pred = obs_unnormalized + unnormalize(delta_pred_normalized.detach().numpy(), delta_std, delta_mean) # TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        prediction = self.forward(obs, acs, data_statistics['obs_mean'], data_statistics['obs_std'], data_statistics['acs_std'], data_statistics['acs_std'], data_statistics['delta_std'], data_statistics['delta_std']  )[0] # TODO(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        :return:
        """
        target = normalize(next_observations-observations, data_statistics['delta_std'], data_statistics['delta_std'])
        ...
        pred = self.forward(observations, actions, data_statistics['obs_mean'], data_statistics['obs_std'], data_statistics['acs_std'], data_statistics['acs_std'], data_statistics['delta_std'], data_statistics['delta_std'])[1]
        loss = self.loss(ptu.from_numpy(target), pred)
