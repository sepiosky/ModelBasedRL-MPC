...
class MPCPolicy(BasePolicy):
    ...
    def evaluate_candidate_sequences(self, candidate_action_sequences, obs):
        ...
        ensembel_avg_rewards = []
        for model in self.dyn_models:
            model_obs, rewards = np.tile(obs, (self.N,1)), np.repeat(0.0, self.N)
            for s in range(self.horizon):
                accs = candidate_action_sequences[:,s,:]
                rewards += self.env.get_reward(model_obs, accs)[0]
                model_obs = model.get_prediction(model_obs, accs, self.data_statistics)
            ensembel_avg_rewards.append(rewards)
        return np.mean(np.array(ensembel_avg_rewards), axis=0)
